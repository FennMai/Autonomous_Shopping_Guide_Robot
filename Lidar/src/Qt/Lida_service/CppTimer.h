...
public:
    ...
    virtual void stop() {
    if (!running) {
        qDebug() << "Timer is not running, no need to stop.";
        return;
    }
    qDebug() << "Stopping timer...";
	running = false;
    if (uthread.joinable()) {
        qDebug() << "Joining timer thread...";
        uthread.detach();
        //uthread.join();  // 等待线程完成
        qDebug() << "Timer thread joined successfully.";
    } else {
        qDebug() << "Timer thread is not joinable, cannot join.";
    }
    }
